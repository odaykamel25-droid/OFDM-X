...
# v11: corrected contextual Actor-Critic adaptive switching
# ============================================================
#
...
class ActorCritic:
    """
    Contextual Actor-Critic policy.

    The state contains:
      - normalized Doppler
      - one-hot path-count class (1/3/7 paths)
      - Doppler/path interaction terms
      - bias

    This avoids the earlier linear-state problem in which the actor could
    converge to AFDM for all states even when OFDM and AFDM had equal BER.
    The decision is still learned from measured paired BER rewards; there
    is no waveform-selection rule hard-coded into the evaluator.
    """

    def __init__(self):
        self.actor_lr = 0.045
        self.critic_lr = 0.020

        # Features:
        # [fd, path1, path3, path7, fd*path3, fd*path7, bias]
        self.n_features = 7

        # Start from an unbiased policy: P(OFDM)=P(AFDM)=0.5.
        self.W = rng.normal(0.0, 0.01, size=(self.n_features, 2))
        self.b = np.zeros(2)

        self.VW = np.zeros(self.n_features)
        self.Vb = 0.0

    def state(self, fd, paths):
        fdn = float(np.clip(fd / 2000.0, 0.0, 1.0))

        p1 = 1.0 if int(paths) == 1 else 0.0
        p3 = 1.0 if int(paths) == 3 else 0.0
        p7 = 1.0 if int(paths) == 7 else 0.0
    ...
    def update_full_information(self, s, rewards):
        """
        Full-information one-step Actor-Critic update.

        Both actions are evaluated on paired measured BERs, so the actor
        receives a contextual reward for OFDM and AFDM at every state.
        """
        ...
        # Actor update.
        # grad E_pi[r] / grad logits = p * (r - E_pi[r])
        advantage = rewards - expected_reward
...
def reward_from_ber(
    ...
    """
    Convert the *measured* paired BERs into contextual utilities.

    The only prior is a small implementation-complexity regularization
    against AFDM. It is not a waveform-selection rule: AFDM is selected
    whenever its measured reliability benefit is sufficiently large.
    ...
    # Reliability utility: lower measured BER is better.
    u_ofdm = -np.log10(
        float(ber_ofdm) + floor
    )

    u_afdm = (
        -np.log10(
            float(ber_afdm) + floor
        )
        - 0.50
    )
    ...
    r_ofdm = u_ofdm - mean_u
    r_afdm = u_afdm - mean_u

    if int(action) == 0:
        return float(
            np.clip(r_ofdm, -1.0, 1.0)
        )

    return float(
        np.clip(r_afdm, -1.0, 1.0)
    )
...
def train_agent():
    ...
    print("\nTraining Actor-Critic from measured paired BER...")
    print("The training set is balanced across 1-, 3-, and 7-path states.")

    # Balanced contextual training:
    # every episode contains one of the three path classes, so the actor
    # cannot ignore the low-complexity 1-path state.
    for ep in range(1, TRAIN_EPISODES + 1):

        fd = float(
            rng.choice(DOPPLER_SWEEP)
        )

        paths = int(
            PATH_CLASSES[
                (ep - 1) % len(PATH_CLASSES)
            ]
        )

        # Small randomization around the sweep grid improves generalization.
        fd = float(
            np.clip(
                fd + rng.uniform(-25.0, 25.0),
                0.0,
                2000.0,
            )
        )
        ...
        agent.update_full_information(
            s,
            [r0, r1],
        )

        if ep % 100 == 0:
            p_after = agent.probs(s)
            name = (
                "OFDM"
                if action == 0
                else "AFDM"
            ...
            print(
                f"Episode {ep:4d}/{TRAIN_EPISODES} | "
                f"fd={fd:7.1f} Hz | "
                f"paths={paths} | "
                f"OFDM BER={b0:.6f} | "
                f"AFDM BER={b1:.6f} | "
                f"action={name:4s} | "
                f"P(AFDM)={p_after[1]:.3f} | "
                f"R0={r0:+.3f} R1={r1:+.3f}"
            )

    # Final learned policy diagnostics at the three paper operating points.
    print("\nLearned policy at paper operating points:")
...
def make_figure(agent, results):
    ...
    filename = "Figure13_REAL_ADAPTIVE_OFDM_AFDM_FINAL_v11.png"
...
if __name__ == "__main__":
    ...
    print(
        "REAL OFDM-X ADAPTIVE WAVEFORM SIMULATION - v11"
    )
