...
def apply_channel(tx, ebn0_db, channel):
    ...
    if channel=="AWGN":
        ...
        # Use ONE fixed physical channel realization for all Eb/N0
        # points.  This prevents random channel changes from making
        # the BER curve non-monotonic and makes OFDM vs OFDM-X fair.
        # The random seed is reset here so OFDM and OFDM-X use the
        # same channel realization and the same underlying noise
        # samples at each Eb/N0 point.
        np.random.seed(VEHICULAR_MAIN_SEED)
...
plt.savefig(
    "Figure11_BER_600dpi_TimesNewRoman_8pt.png",
    dpi=600,
    facecolor="white"
)
