...
# ------------------------------------------------------------
# Publication-quality Figure 7 - FINAL CLEAR VERSION
# ------------------------------------------------------------
# The data and selected waveform are unchanged.
# Only the plotting/layout specifications are improved.
# ------------------------------------------------------------
...
# Clearer curves while keeping the original two datasets.
ax.plot(
    n,
    mag_original,
    linewidth=1.35,
...
# Give the waveform a small amount of headroom so the highest
# peak is not touching the upper frame.
ymax = max(
    np.max(mag_original),
    np.max(mag_fcae)
)
...
# Legend only contains the two waveform names.
# This avoids the overlap seen in some other figures.
leg = ax.legend(
    fontsize=8,
    loc="upper right",
    frameon=True,
...
# Fixed margins: preserves the requested 3.25 x 3.25 inch canvas.
fig.subplots_adjust(
    left=0.18,
    right=0.98,
    bottom=0.17,
...
OUT_FILE = (
    ...
    "300dpi_TNR8_FINAL_CLEAR.png"
)

# ------------------------------------------------------------
# FINAL SAVE: preserve EXACTLY the 3.25 x 3.25 inch canvas.
#
# Do NOT use bbox_inches="tight" here. It can crop/recalculate
# the canvas and make the saved PNG look different from the
# figure displayed by Matplotlib.
# ------------------------------------------------------------
...
# Verify the saved raster dimensions.
from PIL import Image
with Image.open(OUT_FILE) as saved_img:
    print(f"Saved raster size: {saved_img.size[0]} x {saved_img.size[1]} pixels")
    print(f"Saved DPI metadata: {saved_img.info.get('dpi', 'not available')}")

# Display the same publication-resolution figure.
plt.show()

print()
print("=" * 72)
print("FIGURE 7 - FINAL CLEAR PUBLICATION VERSION")
print("=" * 72)
print(f"Selected test symbol index = {idx}")
print(f"Conventional OFDM PAPR      = {papr_db(x_original):.3f} dB")
print(f"FCAE / OFDM-X PAPR          = {papr_db(x_fcae_plot):.3f} dB")
