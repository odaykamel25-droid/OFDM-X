...
# ==========================================================
# REPRODUCIBILITY / DETERMINISTIC FCAE TRAINING
# ==========================================================
# Keep the random streams fixed so repeated executions use
# the same initialization, shuffling, and validation data.
# This does not change the FCAE architecture, loss, channel,
# or any simulation parameter.
# ==========================================================
...
# Reset the same seed immediately before model initialization.
np.random.seed(REPRO_SEED)
random.seed(REPRO_SEED)
tf.random.set_seed(REPRO_SEED)
...
# Reset the same seed immediately before FCAE training.
np.random.seed(REPRO_SEED)
random.seed(REPRO_SEED)
tf.random.set_seed(REPRO_SEED)
...
# ----------------------------------------------------------
# Exact independent-validation PAPR check
# ----------------------------------------------------------
validation_papr_exact = np.mean(
    10
    * np.log10(
        np.max(
...
validation_original_exact = np.mean(
    10
    * np.log10(
        np.max(
            np.abs(s_val_final) ** 2,
            axis=1
        )
        /
        (
...
print("\nExact independent-validation PAPR:")
print(f"Original PAPR = {validation_original_exact:.3f} dB")
print(f"AE PAPR       = {validation_papr_exact:.3f} dB")
print(
    f"Improvement   = "
    f"{validation_original_exact - validation_papr_exact:.3f} dB"
)
