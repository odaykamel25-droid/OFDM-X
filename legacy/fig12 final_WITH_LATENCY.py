...
# ------------------------------------------------
# 23A. REAL FCAE INFERENCE LATENCY MEASUREMENT
# ------------------------------------------------
# IMPORTANT:
# The latency is measured directly from the SAME trained
# `model` object used above for the independent-test prediction.
# No model reload and no retraining are performed here.
#
# This avoids Keras .keras deserialization issues and ensures
# that the measured latency belongs to the FCAE V8 architecture
# used by this script.
# ------------------------------------------------
...
print()
print("=" * 72)
print("FCAE V8 - REAL INFERENCE LATENCY MEASUREMENT")
print("=" * 72)
...
print(f"Input dimension  : {test_x.shape[1]}")
print(f"Independent test : {test_x.shape[0]} samples")
print(f"Batch size       : {BATCH_SIZE}")

# ------------------------------------------------
# Use the SAME independent-test data already used to obtain
# pred_test and the final PAPR results.
# ------------------------------------------------
x_single = tf.convert_to_tensor(
    test_x[:1],
    dtype=tf.float32
)
...
# ------------------------------------------------
# Warm-up.
# Excluded from timing because TensorFlow may perform graph
# tracing and runtime initialization on first calls.
# ------------------------------------------------
print("\nWarming up TensorFlow...")

for _ in range(100):
    y = fcae_forward(x_single)
    _ = y.numpy()

for _ in range(100):
    y = fcae_forward(x_batch)
    _ = y.numpy()
...
# Timing function.
# ------------------------------------------------
def measure_latency(x, runs=1000):
...
# ------------------------------------------------
# Measure one OFDM waveform.
# ------------------------------------------------
print("\nMeasuring batch size = 1...")

lat_single = measure_latency(
    x_single,
    runs=1000
)

# ------------------------------------------------
# Measure the same BATCH_SIZE used by the FCAE V8 training
# and independent-test prediction.
# ------------------------------------------------
print(
    f"Measuring batch size = {BATCH_SIZE}..."
)

lat_batch = measure_latency(
    x_batch,
    runs=1000
)
...
# Derived values.
# ------------------------------------------------
median_per_symbol_ms = (
    lat_batch["median"]
    /
    float(BATCH_SIZE)
)

throughput_symbols_s = (
    float(BATCH_SIZE)
    /
    (
        lat_batch["median"]
...
for key in (
    ...
    print(
        f"{key.capitalize():<10}: "
        f"{lat_batch[key]:.6f} ms"
    )

print()
print(
    "Median latency per OFDM symbol from "
    f"batch={BATCH_SIZE}: "
    f"{median_per_symbol_ms:.6f} ms"
)

print(
    "Approximate throughput: "
    f"{throughput_symbols_s:.2f} OFDM symbols/s"
)

# ------------------------------------------------
# Save measured latency.
# ------------------------------------------------
LATENCY_FILE = (
    "FCAE_OFDM_X_V8_INFERENCE_LATENCY_RESULTS.txt"
)
...
with open(
    ...
    f.write(
        "FCAE / OFDM-X V8 - REAL INFERENCE LATENCY\n"
    )
    f.write("=" * 72 + "\n")
    f.write(
    ...
    f.write(
        f"Devices: {tf.config.list_physical_devices()}\n"
    )
    f.write(
        f"Model parameters: {model.count_params():,}\n"
    )
    f.write(
        f"Input dimension: {test_x.shape[1]}\n"
    )
    f.write(
        f"Independent test samples: {test_x.shape[0]}\n"
    )
    f.write(
    ...
    for key, value in lat_single.items():
        f.write(
            f"{key}: {value:.6f} ms\n"
        )
    ...
    for key, value in lat_batch.items():
        f.write(
            f"{key}: {value:.6f} ms\n"
        )

    f.write(
        "\nMedian latency per OFDM symbol: "
        f"{median_per_symbol_ms:.6f} ms\n"
    )
...
print(
    f"\nLatency results saved to: {LATENCY_FILE}"
)
