...
def rayleigh_channel_ici(
    ...
    # Time within each transmitted OFDM symbol (including CP)
    ...
    # Time-varying, delayed channel taps:
    # h_i[n,l] = path gain at output time n for delay l.
    h_taps = np.zeros(
        (
            num_symbols,
            total_len,
    ...
    # ------------------------------------------------------
    # Diagonal frequency response only.
    # This is NOT a full ICI equalizer; it is used only
    # to quantify the residual ICI after one-tap EQ.
    # ------------------------------------------------------
    ...
    for i in range(num_symbols):
        ...
        for k in range(N):
            ...
            diagonal_gain = np.sum(
                instantaneous_h * phase[None, :],
                axis=1
            )
...
for eb in [0, 8, 12, 16, 20]:
    ...
    # Diagnostic diagonal frequency response only.
    # Diagnostic diagonal frequency response only
# Keep exactly N subcarriers
H_diag = np.fft.fft(
    h_time,
    n=N,
    axis=1
